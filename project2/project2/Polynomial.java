...
public class Polynomial implements Iterable<Polynomial.Term>, Comparable<Polynomial> {
	...
	public Polynomial(String fromFile) {
		head = null; //set head to null, return null at the end of script. 
		try (Scanner termScanner = new Scanner(fromFile)) {
			try{
 ...
 * 
 * FUNCTION: 
 * First compares exponents, then coefficients. A positive value indicate this is 
...
public int compareTo(Polynomial otherPoly) {
	...
	}	//if both are null, and at this point, they are equal, reset to zero
...
/* METHOD: compareExponents AND  Arguments TO Another Polynomial object
 * FUNCTIONS: compares exponents in two polynomials, is called to check weak order
 * 
 * */
...
//COMPARES POLYNOMIALS TO EXPONENTS
...
/*Suppression due to iterator not being checked
 ...
 * Function: using StringBuilder, traverses the the expression
 * adding the the string value of the term.
 * 
...
public String toString() {
	...
	//first check head to avoid certain parameters
...
/*Nested Class: Term and arguments:
 *Double coefficient and integer exponent or overridden toString
 *
 *Function: nodes of the polynomial objects which carry the reference to the next node
 * 
 * */
...
private Term(double c, int e) {
	coefficient = c;
	exponent = e;
	next = null; // initially setting to null 
}
